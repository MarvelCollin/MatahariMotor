...
const Navbar = () => (
  <header data-header className="group sticky top-0 z-50 bg-ink text-white">
    <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
      ...
      >
        <span className="group-data-open:hidden">Menu</span>
        <span className="hidden group-data-open:inline">Tutup</span>
      </button>
    </div>

    <div className="grid grid-rows-[0fr] overflow-hidden transition-[grid-template-rows] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] group-data-open:grid-rows-[1fr] motion-reduce:transition-none md:hidden">
      <nav id="mobile-menu" inert className="min-h-0 overflow-hidden" aria-label="Menu mobile">
        <div className="border-t border-white/15 px-4 pb-4">
          {links.map((l) => (
            <a key={l.href} href={l.href} className="block border-b border-white/10 py-3.5 text-lg last:border-b-0">
              {l.label}
            </a>
          ))}
        </div>
